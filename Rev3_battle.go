...
func main(){
	...
	//incase error rededclare value here
	condition := false
	status := false
	jumlah = -1
    stk := false
	...
	for i := 0; i < 10; i++ {
		...
		condition, arrayplayer[jumlah].slot, arrayplayer[jumlah].perk, arrayplayer[jumlah].start, stk = battle(arrayplayer[jumlah].nama,arraystat[jumlah].hp, arraystat[jumlah].dmg, ehp, edmg, opt, arraystat[jumlah].heal, eheal,arrayplayer[jumlah].perk, arrayplayer[jumlah].slot, arrayplayer[jumlah].start)
	if condition == true {
		fmt.Println("You won")
		if stk == true {
			fmt.Println("You have received", ehp*40/100, "Hp", edmg*40/100, "Dmg", eheal*40/100, "Heal from Thanos")
			arraystat[jumlah].hp += ehp * 40 / 100
			arraystat[jumlah].dmg += edmg * 40 / 100
			arraystat[jumlah].heal += eheal * 40 / 100
		}
		rng = rand.Intn(3)
	   ...
	   arrayplayer[jumlah].perk = arrayplayer[jumlah].perk + rng + 12
		...
		eheal  += rand.Intn(2) + 2
		if i <= 8 {
        arrayplayer[jumlah].perk,arraystat[jumlah].hp, arraystat[jumlah].dmg, arraystat[jumlah].heal = shop(arrayplayer[jumlah].perk, arraystat[jumlah].hp, arraystat[jumlah].dmg, arraystat[jumlah].heal)
		}
	} else {
